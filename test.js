...
test = satisfies({ name: /^[A-Z]/, alive: true })
assert.equal(test({ alive: true }), false)
assert.equal(test({ name: 'michael', alive: true }), false)
assert.equal(test({ name: 'Michael', alive: false }), false)
assert.equal(test({ name: 'Michael', alive: true }), true)
